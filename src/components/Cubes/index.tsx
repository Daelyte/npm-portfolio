...
const Cubes: React.FC<CubesProps> = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center pointer-events-none" style={wrapperStyle}>
      <div ref={sceneRef} className="grid w-full h-full pointer-events-auto" style={sceneStyle}>
        {cells.map((_, r) =>
          cells.map((__, c) => (
            <div
              key={`${r}-${c}`}
