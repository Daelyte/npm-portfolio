...
const SingleProject = ({ project }: { project: Project }) => {
  return (
    <a
      href={project.link}
      target="_blank"
      className="block p-6 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl shadow-xl hover:bg-white/10 transition-all duration-300 relative group focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-900"
      aria-label={`View ${project.name} project`}
    >
      <h3 className="text-2xl font-semibold pb-2 text-white group-hover:text-blue-300 transition-colors">
        {project.name}
      </h3>
      <p className="py-2 text-gray-300 leading-7 text-base">{project.desc}</p>

      <svg
        className="w-8 h-8 rotate-180 ml-auto absolute bottom-3 right-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform duration-300"
        viewBox="0 0 24 24"
        fill="none"
